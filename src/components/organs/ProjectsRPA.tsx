...
import { RPAProjects } from "../particles/Data";
import img1 from "../../assets/images/RPA/Edubotix.png" 
import img2 from "../../assets/images/RPA/HRautomation.jpg" 
import img3 from "../../assets/images/RPA/Hackfo.jpg" 
import img4 from "../../assets/images/RPA/Meeted.jpg" 
import img5 from "../../assets/images/RPA/Internbotix.jpg" 
import img6 from "../../assets/images/RPA/Accubotix.jpg" 
        ...
        <section className="w-full h-auto flex items-center bg-zinc-950">
            <main className="w-full lg:h-[700px] md:h-[600px] flex flex-col justify-center items-center gap-10 md:gap-10 py-12 md:py-0">
                ...
                <div className="flex flex-col mt-10 items-center relative before:absolute before:-bottom-6 before:left-30 before:w-20 before:h-1 before:rounded-lg before:bg-gradient-to-r before:from-teal-500 before:to-pink-500 z-10">
                    <Text as="p" className="text-teal-600 lg:text-sm text-xs tracking-widest uppercase font-medium">{RPAProjects.firstText}</Text>
                    <Text as="h1" className="text-zinc-100 lg:text-5xl md:text-4xl text-3xl">{RPAProjects.secondText}</Text>
                ...
                <div className="w-full lg:px-8 md:px-2 h-auto mt-4">
                    <Slider {...settings}>
                        {
                            RPAProjects.projects.map((feedback, index) => (
                                <div key={index} className="lg:px-6 md:px-4 w-full px-3">
                                    <Card className="bg-zinc-800 w-full h-[350px] grid grid-cols-3 border-[0.1rem] border-pink-500/30">
                                        <div className="flex flex-col justify-center md:justify-start gap-1 col-span-2 lg:p-8 p-4">
                                            <Text as="h2" className="md:text-xl text-lg font-semibold text-cyan-500">{feedback.title}</Text>
                                            <Text as="h6" className="text-zinc-300 text-sm pb-2">{feedback.oneline}</Text>
                                            <Text as="h2" className="text-amber-300 text-sm uppercase">{feedback.description}</Text>
                                            <Text as="h1" className="mt-3 text-sm text-zinc-300">
                                                {feedback.tools}
                                            </Text>
                                        </div>
