...
import { ArrowCircleRight, Alien, CirclesFour } from "@phosphor-icons/react";
...
const NavBar = () => {
    ...
    return (
        <header className="w-full h-auto bg-transparent overflow-x-hidden fixed z-50 top-0 left-0">
            ...
            <Link to={`/`} className="font-extrabold flex items-center relative md:text-2xl text-lg" onClick={() => window.scrollTo(0, 0)}>
                    ...
                    <ul className="flex items-center justify-center h-full gap-4 relative before:w-full before:h-0.5 before:absolute before:bottom-0 before:left-0 before:bg-zinc-400">
                        {
                            NavLinks.map((navlink, index) => (
                                <List className="w-full text-base " key={index}>
                                    <NavLink to={navlink.url} className={`relative inline-block text-cyan-400  px-2 whitespace-nowrap text-white uppercase text-xs font-extrabold transition-all duration-200 hover:text-violet-500 before:w-0 before:h-0.5 before:bg-gradient-to-r from-fuchsia-500 to-cyan-500 before:absolute before:-bottom-[2.93rem] before:left-0 before:transition-all before:duration-200 before:ease-in hover:before:left-0.5`}>{navlink.name}</NavLink>
                                </List>
                            ))
                        }
                    ...
                    <section className="w-full px-4 py-6 flex flex-col gap-16">
                        <div className="w-full flex pt-5 px-4 justify-between items-center">
                            <Link to={`/`} className="font-extrabold text-2xl">
                                <Text as="span" className="bg-gradient-to-r from-fuchsia-600 to-blue-500 bg-clip-text text-transparent">EVOL</Text>
                                <Text as="span" className="bg-gradient-to-r from-blue-500 to-cyan-500 bg-clip-text text-transparent">iiON</Text>
                                <Text as="span" className="text-lime-500 absolute top-5 left-20"><Alien size={23} color="currentColor" weight="fill" /></Text>
                            </Link>
                        ...
                        <ul className="flex flex-col gap-3 pl-5">
                            {
                                NavLinks.map((navlink, index) => (
                                    <List className="w-full text-base" key={index}>
                                        <NavLink to={navlink.url} onClick={handleToggle} className={`relative overflow-hidden inline-block text-white before:w-full before:h-0.5 before:bg-color2 before:absolute hover:text-violet-500 before:bottom-0 before:-left-full before:rounded-full before:transition-all before:duration-200 before:ease-in hover:before:left-0`}>{navlink.name}</NavLink>
                                    </List>
                                ))
                            }
                        </ul>
