...
import Offer from "../../assets/images/Project.png"
import { Text } from "../atoms/Text"
import { ProjectService } from "../particles/Data"
import { List } from "../atoms/List"
import { useCallback } from "react"
import { Lightbulb, BookOpen, Graph } from "@phosphor-icons/react"
...
const AboutProjects = () => {
    ...
    const renderIcon = useCallback((element: number) => {
        switch (element) {
            case 0:
                return <Lightbulb size={50} className="text-fuchsia-500" weight="light" />;
            case 1:
                return <BookOpen size={50} className="text-fuchsia-500" weight="light" />;
            case 2:
                ...
                <div className="h-full w-full lg:px-10 px-4 flex flex-col lg:justify-center md:justify-end justify-center items-start md:gap-20 lg:gap-24 gap-16">
                    <Fade className="w-full">
                        <div className="w-full flex flex-col mt-10 lg:mt-24 items-center relative before:absolute before:-bottom-6 before:left-38 before:w-20 before:h-1 before:rounded-lg before:bg-gradient-to-r before:from-emerald-400 before:to-cyan-400 z-10">
                            <Text as="p" className="text-emerald-400 lg:text-sm text-xs tracking-widest uppercase font-medium">{ProjectService.firstText}</Text>
                            <Text as="h1" className="text-zinc-100 lg:text-5xl md:text-4xl text-3xl">{ProjectService.secondText}</Text>
                        ...
                        <ul className="flex flex-col lg:gap-8 gap-6 pb-16">
                            {
                                ProjectService.list.map((item, index) => (
